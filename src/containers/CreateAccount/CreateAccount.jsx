...
class CreateAccount extends Component {
    ...
    validationEmail = (e) => {
        if (e.target.value !== '') {
            if (validateEmail(e.target.value)) {
                ...
                axios
                    .post('https://zieglerlabs-form-validator.herokuapp.com/check-email',
                        {
                    ...
                    .then((response) => {
                        this.setState({email: e.target.value, emailError: !response.data.available ? 'Email busy' : ''})
                    })
            } else
    ...
    validationName = (e) => {
        if (e.target.value !== '')
            ...
            this.setState({name: e.target.value, nameError: 'Enter the name'})
    }
    ...
    validationPassword = (e) => {
        if (e.target.value !== '') {
            if (validatePassword(e.target.value))
                ...
                this.setState({password: e.target.value, passwordError: 'Wrong password'})
        } else
            this.setState({password: e.target.value, passwordError: 'Enter the password'})
    }
    ...
    render() {
        const {nameError, emailError, passwordError, password, name, email, date} = this.state
        return (
            <Paper elevation={3} className={styles.container}>
                ...
                <Button variant="contained"
                        ...
                        disabled={!(!nameError && !emailError && !passwordError && email && name && password)}
